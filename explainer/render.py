...
def frame(base, t, total, index, speaker, caption, amplitude, progress):
    ...
    titles=['A focused reading queue.','From search to your library.',
            ...
            'Keep your reading queue.','Read. Compare. Verify.']
    ...
    if index==0:
        ...
        text(d,(652,304),'SAVED AND SEARCHABLE',13,COPPER,True,'ma')
        for i,name in enumerate(['Library','Reports','Alerts']):
            x=404+i*181
            box(d,(x,351,x+154,405), '#edf4f0' if int(progress*3)%3==i else '#fffefd')
            text(d,(x+77,366),name,22,bold=True,anchor='ma')
        text(d,(652,433),'Keep selected papers in one reading queue.',15,TEAL,True,'ma')
        text(d,(652,470),'Open the source record and read critically.',16,MUTED,anchor='ma')
        text(d,(652,505),'GitHub notifications · Daily reports · Searchable library',12,MUTED,anchor='ma')
    else:
